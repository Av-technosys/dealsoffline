...
const BeautyMenu = ({ path, setFilterValueList, filterValueList }) => {
  const [selectedItems, setSelectedItems] = useState(() => {
    return Menus[3].subMenu
  ...
  useEffect(() => {
    ...
    if (selectedItems.length > 0) {
      setFilterValueList((prev) => ({
        ...prev,
        categories: prev.categories.includes("Beauty")
          ? prev.categories
          : [...prev.categories, "Beauty"],
      }));
    } else {
      setFilterValueList((prev) => ({
        ...prev,
        categories: prev.categories.filter((item) => item !== "Beauty"),
      }));
    }
  ...
  useEffect(() => {
    if (!filterValueList.includes("Beauty") && selectedItems.length > 0) {
      setSelectedItems([]);
    }
  }, [filterValueList]);
      ...
      <DialogTrigger>
        <div
          className={`border h-9 flex items-center gap-1 border-black px-3 py-2 text-xs rounded font-semibold text-black ${
            selectedItemsLength > 0 ? "bg-secondary-red" : " bg-transparent"
          }`}
        >
          Beauty
      ...
      <DialogContent showCloseButton={true} className="max-w-5xl w-full">
        ...
        >
          ...
          <div className=" w-full">
            ...
            >
              {Menus[3].subMenu.map((submenu, index) => (
