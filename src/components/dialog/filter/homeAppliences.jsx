...
const HomeAppliencesMenu = ({ path, setFilterValueList, filterValueList }) => {
  ...
  useEffect(() => {
    ...
    if (selectedItems.length > 0) {
      setFilterValueList((prev) => ({
        ...prev,
        categories: prev.categories.includes("Home Appliances")
          ? prev.categories
          : [...prev.categories, "Home Appliances"],
      }));
    } else {
      setFilterValueList((prev) => ({
        ...prev,
        categories: prev.categories.filter(
          (item) => item !== "Home Appliances"
        ),
      }));
    }
  }, [selectedItems]);
  ...
  useEffect(() => {
    if (
      !filterValueList.includes("Home Appliances") &&
  ...
  return (
    <Dialog>
      <DialogTrigger>
        ...
        >
          Home Appliances
          <img src="./../home-appliences.svg" className="size-5" alt="" />
        </div>
      </DialogTrigger>
      <DialogContent showCloseButton={true} className="max-w-5xl w-full">
        ...
        >
          ...
          <div className=" w-full">
            ...
            >
              {Menus[5].subMenu.map((submenu, index) => (
...
export default HomeAppliencesMenu;
