...
const MenuKids = ({ path, setFilterValueList, filterValueList }) => {
  const [selectedItems, setSelectedItems] = useState(() => {
    return Menus[2].subMenu
  ...
  useEffect(() => {
    ...
    if (selectedItems.length > 0) {
      setFilterValueList((prev) => ({
        ...prev,
        categories: prev.categories.includes("Kids")
          ? prev.categories
          : [...prev.categories, "Kids"],
      }));
    } else {
      setFilterValueList((prev) => ({
        ...prev,
        categories: prev.categories.filter((item) => item !== "Kids"),
      }));
    }
  }, [selectedItems]);

  useEffect(() => {
    if (!filterValueList.includes("Kids") && selectedItems.length > 0) {
      setSelectedItems([]);
    }
  }, [filterValueList]);
